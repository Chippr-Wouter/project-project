...
describe.runIf(process.env.DATABASE_URL !== undefined)(
  ...
  () => {
    ...
    it.scoped("queries, filters, paginates, and counts indexed tickets", () =>
      Effect.gen(function* () {
        const suffix = randomUUID()
        ...
        const januaryTenth = DateTime.toDate(
          DateTime.unsafeMake("2026-01-10T00:00:00.000Z")
        )
        const januaryEleventh = DateTime.toDate(
          DateTime.unsafeMake("2026-01-11T00:00:00.000Z")
        )
        const januaryTwelfth = DateTime.toDate(
          DateTime.unsafeMake("2026-01-12T00:00:00.000Z")
        )
        const documents = [
          indexedDocument("T-2", {
            status: ticketStatus("todo"),
