...
- **`beginProfileConnect`** builds `https://www.figma.com/oauth?client_id=…&redirect_uri=…&scope=…&state=…&response_type=code` with scopes `current_user:read file_content:read file_metadata:read file_dev_resources:read file_dev_resources:write`, and a cryptographically random `state` persisted for single use.
